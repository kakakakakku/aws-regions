...
func main() {
	...
	app.Action = func(c *cli.Context) {
		...
		rs = append(rs, Region{"eu-west-3", "EU (Paris)", "欧州（パリ）", "3 AZs"})
		rs = append(rs, Region{"eu-north-1", "EU (Stockholm)", "欧州（ストックホルム）", "3 AZs"})
		rs = append(rs, Region{"ap-east-1", "Asia Pacific (Hong Kong)", "アジアパシフィック（香港）", "3 AZs"})
		...
		rs = append(rs, Region{"us-gov-west-1", "AWS GovCloud (US)", "AWS GovCloud (米国西部)", "3 AZs"})
